...
export async function GET(
  ...
  try {
    ...
    if (includeLedger && 'ledgerEntries' in wallet) {
      const ledgerEntries = wallet.ledgerEntries as Array<{
        id: string;
        entryType: string;
        status: string;
        ...
        pendingBalanceAfter: { toNumber?: () => number } | number;
        availableBalanceAfter: { toNumber?: () => number } | number;
      ...
      response.ledgerEntries = ledgerEntries.map(e => ({
        id: e.id,
        entryType: e.entryType,
        status: e.status,
        amount: typeof e.amount === 'number' ? e.amount : Number(e.amount),
        currency: e.currency,
        balanceAfter: typeof e.balanceAfter === 'number' ? e.balanceAfter : Number(e.balanceAfter),
        pendingBalanceAfter: typeof e.pendingBalanceAfter === 'number' ? e.pendingBalanceAfter : Number(e.pendingBalanceAfter),
        availableBalanceAfter: typeof e.availableBalanceAfter === 'number' ? e.availableBalanceAfter : Number(e.availableBalanceAfter),
