...
export async function GET(request: NextRequest) {
  try {
    ...
    const feeSummary = await educationPortalService.getFeeSummary(tenantId, studentId);

    const serializedData = {
      ...feeSummary,
      feeBreakdown: feeSummary.feeBreakdown.map(fee => ({
        ...fee,
    ...
    console.error('Education portal fees error:', error);
