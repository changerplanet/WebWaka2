...
export async function GET(request: NextRequest) {
  try {
    ...
    const appointments = await healthPortalService.getAppointments(tenantId, patientId, includeUpcoming);

    const serializedData = appointments.map(apt => ({
      ...apt,
    ...
    console.error('Health portal appointments error:', error);
