...
const DEMO_PRODUCTS: SVMProduct[] = [
  {
    ...
    description:
      "A timeless classic white t-shirt made from 100% organic cotton. Perfect for everyday wear.",
  ...
  },
  {
    ...
    slug: "denim-jacket",
    description:
      "Classic denim jacket with a modern fit. Features brass buttons and multiple pockets.",
  ...
  },
  {
    ...
    description:
      "Lightweight running sneakers with superior cushioning and breathable mesh upper.",
  ...
  },
  {
    ...
    slug: "leather-wallet",
    description:
      "Handcrafted genuine leather wallet with RFID protection. Multiple card slots and bill compartment.",
    shortDescription: "Premium leather bifold wallet",
    categoryId: "cat-3",
    categoryName: "Accessories",
    tags: ["leather", "wallet", "gift"],
    basePrice: 49.99,
    compareAtPrice: null,
    currency: "USD",
    images: [
  ...
  },
  {
    ...
    description:
      "Durable canvas backpack with laptop compartment. Water-resistant and perfect for travel.",
    shortDescription: "Travel-ready canvas backpack",
    categoryId: "cat-3",
    categoryName: "Accessories",
    tags: ["bags", "travel", "canvas"],
    basePrice: 69.99,
    compareAtPrice: null,
    currency: "USD",
    images: [
    ...
    status: "ACTIVE",
    hasVariants: true,
    options: [{ name: "Color", values: ["Navy", "Olive", "Gray"] }],
    variants: [
      {
        id: "var-6a",
        productId: "prod-6",
...
const DEMO_SHIPPING_OPTIONS: ShippingOption[] = [
  {
    ...
    amountToFreeShipping: 44.01,
    estimatedDays: { min: 5, max: 7 },
  },
  {
    rateId: "ship-2",
    zoneName: "Domestic",
    rateName: "Express Shipping",
    carrier: "FedEx",
    fee: 12.99,
    isFree: false,
    freeShippingApplied: false,
...
export function SVMProvider({ children, tenantId }: SVMProviderProps) {
  ...
  if (!tenantId) {
    throw new Error(
      "SVMProvider requires a tenantId. No tenant context available.",
    );
  }
  ...
  useEffect(() => {
    const savedCart = loadFromStorage<Cart>(STORAGE_KEYS.CART, EMPTY_CART);
    if (savedCart && savedCart.items && savedCart.items.length > 0) {
      setCart(savedCart);
    }
    const savedAddress = loadFromStorage<ShippingAddress | null>(
  ...
  const recalculateCart = useCallback(
    (items: CartItem[], promoCode?: string | null, shipping?: number) => {
    ...
    async (query?: string, categoryId?: string) => {
      ...
      try {
        const res = await fetch(
          `/api/svm/catalog?tenantId=${tenantId}${query ? `&query=${encodeURIComponent(query)}` : ""}${categoryId ? `&categoryId=${categoryId}` : ""}&limit=24`,
        );
  ...
  const fetchProduct = useCallback(
    async (productId: string): Promise<SVMProduct | null> => {
      try {
        const res = await fetch(
          `/api/svm/catalog?tenantId=${tenantId}&productId=${productId}`,
        );
        const data = await res.json();
  ...
  const addToCart = useCallback(
    async (product: SVMProduct, variant?: SVMVariant, quantity = 1) => {
      ...
      try {
        ...
        if (data.success && data.options?.length > 0) {
          ...
          const options = data.options.map((opt: ShippingOption) => ({
            ...opt,
            amountToFreeShipping: opt.freeShippingApplied
              ? 0
              : cart.subtotal >= 50
                ? 0
                : 50 - cart.subtotal,
  ...
  const selectShipping = useCallback(
    (option: ShippingOption) => {
      ...
      setCart((prev) => {
        const calculated = recalculateCart(
          prev.items,
          prev.promotionCode,
          option.fee,
        );
  ...
  return <SVMContext.Provider value={value}>{children}</SVMContext.Provider>;
}
