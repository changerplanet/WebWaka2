  ...
  static async earnPoints(tenantId: string, input: EarnInput, createdBy?: string) {
    ...
    await prisma.customer.update({
      ...
      data: {
        ...
        loyaltyTier: this.calculateTier(currentBalance + input.points, parseJsonField(program.tierConfig, TierConfigSchema, DEFAULT_TIER_CONFIG)),
      },
    });
  ...
  static async awardBonus(
    ...
    await prisma.customer.update({
      ...
      data: {
        ...
        loyaltyTier: this.calculateTier(currentBalance + points, parseJsonField(program.tierConfig, TierConfigSchema, DEFAULT_TIER_CONFIG)),
      },
    });
