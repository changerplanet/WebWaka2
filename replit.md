...
- **Nigeria-First Modular Commerce**:
    ...
    - **Multi-Vendor Customer Cart (Wave K.1)**: Production-grade multi-vendor cart system for MVM marketplace. Features include: mvm_cart and mvm_cart_item database schema, MultiVendorCartService with cart CRUD and vendor validation, 6 API endpoints (get, add, update, remove, clear, prepare-checkout), IndexedDB-based offline cart persistence (MvmOfflineCartService), MultiVendorCartDrawer with vendor-grouped item display and trust badges, cart integration in marketplace header. Constraints: NO payment execution (deferred to K.2), NO order creation, NO automation/background jobs. Security: tenant isolation enforced via TenantContextResolver.resolveForMVM() and explicit tenantId guards in product lookups.
- **SVM Public Storefront Routing (Wave I.1)**: Public-facing storefront access via tenant slug URL patterns (/[tenantSlug]/store and /[tenantSlug]/product/[productSlug]). Features include: server-side tenant resolution by slug, SEO metadata generation (title, meta, canonical), fail-safe store availability gating (only ACTIVE tenants with svm/commerce/store modules), 404 fallback for invalid/suspended tenants, integration of existing SVM components (ProductGrid, ProductDetail, CartDrawer, OfflineCartIndicator, RecentPurchasesTicker). Constraints: NO new business logic (reuses existing SVM infrastructure), NO authentication required for browsing. Known architectural debt: tenantId is exposed to client bundles via SVMProvider - future isolation work would require server-side API proxy layer.
    - **MVM Public Marketplace Routing (Wave I.2)**: Public-facing multi-vendor marketplace access via tenant slug URL patterns. Routes: /[tenantSlug]/marketplace (vendor listing), /[tenantSlug]/marketplace/vendor/[vendorSlug] (vendor storefront), /[tenantSlug]/marketplace/product/[productSlug] (product detail with vendor attribution). Features include: server-side tenant/vendor resolution, SEO metadata generation, vendor trust badges (from Wave G1 rating system), vendor cards with ratings/products counts, product cards with vendor attribution and pricing. Constraints: NO new business logic (reuses existing MVM services), NO new cart/checkout logic (placeholder only), NO automation, NO payouts, NO ranking algorithms, NO schema changes. Known architectural debt: tenantId exposed to client bundles (same as Wave I.1).
    - **ParkHub Public Operator Marketplace (Wave I.3)**: Public-facing transport marketplace access via tenant slug URL patterns. Routes: /[tenantSlug]/parkhub (landing), /[tenantSlug]/parkhub/operators (operator listing), /[tenantSlug]/parkhub/operator/[operatorSlug] (operator profile), /[tenantSlug]/parkhub/routes (route browser). Features include: server-side tenant resolution with ParkHub capability check, SEO metadata generation, operator cards with ratings and route counts, route listings with pricing, booking links to existing /parkhub/booking flow. Data sources: mvm_vendor (as operators), park_route, park_trip. Constraints: NO new business logic, NO new booking/pricing logic, NO schema changes, NO automation. Known gaps: No dedicated operator model (uses mvm_vendor), park_route.parkId references tenant not vendor, missing operator metadata (fleet size, bus types, amenities), missing schedule/timetable data.
    - **Sites & Funnels Public Rendering (Wave I.4)**: Public-facing site and funnel page rendering. Routes: /[tenantSlug] (site homepage), /[tenantSlug]/site/[pageSlug] (published page), /[tenantSlug]/funnel/[funnelSlug] (funnel entry), /[tenantSlug]/funnel/[funnelSlug]/[stepSlug] (funnel step), /[tenantSlug]/form/[formSlug] (form landing). Features include: server-side tenant resolution with Sites & Funnels capability check, SEO metadata generation, BlockRenderer integration, mobile-first design. Constraints: NO new business logic, NO schema changes, NO automation, NO preview modes. Known gaps: No isHomepage flag (using slug='home'), no theme/layout system, domain-based routing not implemented, form public renderer basic, no funnel sequential enforcement (would require session tracking).
    - **Customer Order Portal (Wave I.5)**: Read-only customer order portal for order history and tracking. Routes: /[tenantSlug]/orders (orders index), /[tenantSlug]/orders/[orderRef] (order detail), /[tenantSlug]/orders/ticket/[ticketRef] (ParkHub ticket). Features include: unified order interface (SVM, MVM, ParkHub), NGN currency formatting, status badges, mobile-first design, demo tenant full access, live tenant requires email/phone query param. Constraints: NO new business logic, NO schema changes, NO payments, NO refunds, NO messaging, NO cancellations. Known gaps: No unified order abstraction, inconsistent status enums, missing customer identity linkage, missing receipt association, no secure authentication (email/phone enumeration possible).
