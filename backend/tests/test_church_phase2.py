...
class TestChurchMinistriesAPI:
    ...
    def test_create_ministry(self):
        ...
        if response.status_code == 201:
            data = response.json()
            assert "id" in data or "ministryId" in data
            assert data.get("name") == "Praise & Worship Ministry"
            assert data.get("type") == "CHOIR"
            assert data.get("meetingDay") == "SATURDAY"
    ...
    def test_list_ministries(self):
        ...
        if response.status_code == 200:
            data = response.json()
            # API returns ministries array, not data array
...
class TestChurchDepartmentsAPI:
    ...
    def test_create_department(self):
        ...
        if response.status_code == 201:
            data = response.json()
            assert "id" in data or "departmentId" in data
            assert data.get("name") == "Youth Department"
            assert data.get("code") == "YOUTH"
...
class TestChurchServicesAPI:
    ...
    def test_create_service(self):
        ...
        if response.status_code == 201:
            data = response.json()
            assert "id" in data or "serviceId" in data
            assert data.get("name") == "Sunday Morning Service"
            assert data.get("type") == "SUNDAY_SERVICE"
            assert data.get("dayOfWeek") == 0
...
class TestChurchEventsAPI:
    ...
    def test_create_event(self):
        ...
        if response.status_code == 201:
            data = response.json()
            assert "id" in data or "eventId" in data
            assert data.get("title") == "Annual Revival Crusade"
            assert data.get("type") == "CRUSADE"
...
class TestChurchVolunteerLogsAPI:
    ...
    def test_create_volunteer_log(self):
        ...
        if response.status_code == 201:
            data = response.json()
            assert "id" in data or "logId" in data
            assert data.get("activity") == "Sunday School Teaching"
            assert data.get("hoursServed") == 3
...
class TestChurchTrainingRecordsAPI:
    ...
    def test_create_training_record(self):
        ...
        if response.status_code == 201:
            data = response.json()
            assert "id" in data or "trainingId" in data
            assert data.get("title") == "Children's Ministry Training"
...
class TestChurchSpeakerInvitesAPI:
    ...
    def test_create_speaker_invite(self):
        ...
        if response.status_code == 201:
            data = response.json()
            assert "id" in data or "inviteId" in data
            assert data.get("speakerName") == "Pastor John Adebayo"
